...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger, DropdownMenuLabel, Badge, Sheet, SheetContent, SheetTitle } from "@kosha/ui"
...
// Animation variants for nav entrance (desktop only)
const navVariants = {
...
function NavContent({
  ...
  setIsReportIssueOpen: (v: boolean) => void
  onNavClick?: () => void
}) {
  ...
  return (
    <>
      ...
      <div className="h-16 flex items-center justify-between px-6">
        <Link href="/orders" onClick={onNavClick}>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={onNavClick}
                className={cn(
                  "flex items-center gap-3 px-3 py-3 text-sm font-medium rounded-lg transition-all duration-150",
                  isActive
...
export function MainNav({
  ...
  let sidebar: { isOpen: boolean; close: () => void } = { isOpen: false, close: () => {} }
  try {
    sidebar = useSidebar()
  } catch {
    // SidebarProvider not mounted yet (e.g. during SSR or in pages without it)
  }
  ...
  return (
    <>
      ...
      <Sheet open={sidebar.isOpen} onOpenChange={(open) => !open && sidebar.close()}>
        <SheetContent side="left" className="w-60 p-0 flex flex-col" hideCloseButton>
          <VisuallyHidden.Root>
            <SheetTitle>Navigation</SheetTitle>
          </VisuallyHidden.Root>
          <NavContent {...sharedProps} onNavClick={sidebar.close} />
        </SheetContent>
      </Sheet>
    </>
  )
