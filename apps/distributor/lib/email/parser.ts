...
export interface ParsedOrderData {
  ...
  // Missing info hints from AI (used for clarification emails, but NOT for completeness decision)
  missingInfo: string[]            // e.g., ["Unit price for Chicken", "Quantity for Beef"]

  // Clarification email (generated by AI when fields are missing)
...
${organizationName || 'Our Team'}"${orgSystemPrompt ? `\n\n--- ORGANIZATION INSTRUCTIONS ---\n${orgSystemPrompt}` : ''}${customerHistoryPrompt || ''}${ragExamples && ragExamples.length > 0 ? formatExamplesForPrompt(ragExamples) : ''}`,
    ...
    if (productCatalog && productCatalog.length > 0) {
      items = items.map(item => {
        if (!item.sku) {
          console.log(`📦 Item "${item.name}" has no SKU - skipping catalog normalization`)
          return item
        }
        ...
        if (catalogProduct) {
          ...
          console.log(`✅ SKU "${item.sku}" matched catalog product - normalizing name from "${item.name}" to "${catalogProduct.name}"`)
          return {
            ...item,
        ...
        console.log(`⚠️ SKU "${item.sku}" not found in product catalog (${productCatalog.length} products)`)
        return item
      })
    } else {
      console.log(`⚠️ No product catalog available for normalization (catalog size: ${productCatalog?.length ?? 0})`)
    }
 ...
 * Note: isComplete is NOT included - completeness is determined programmatically, not by AI
 */
...
/**
 * Analyze order and generate clarification email if needed.
 * Note: This does NOT determine completeness - that's done programmatically.
 * This is used to get AI-generated missingInfo hints and clarification emails.
 *
...
- Company name (who is placing the order)

An order is COMPLETE if:
1. Company name is provided
2. ALL items have name, quantity, quantity unit, AND unit price

An order is INCOMPLETE if company name is missing OR any item is missing required fields.

Also check for obvious errors (e.g., quantity of 0, negative prices).
