...
import type { Order, OrderStats } from "@kosha/types"
      ...
      <main className="flex-1 overflow-y-auto md:pl-60">
        <div className="w-full px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
            <div className="bg-slate-100/80 rounded-xl p-1.5 inline-flex overflow-x-auto max-w-full scrollbar-hide">
              {tabs.map((tab) => (
                <TabButton
                  key={tab.key}
                  tabKey={tab.key}
                ...
                <div className="md:hidden contents">
                  {displayedOrders.map((order) => (
                    <OrderCard
                      ...
                      order={order}
                      onClick={() => handleOrderClick(order)}
                      onReject={async (orderId) => { handleRejectClick(order) }}
                      onApprove={handleApprove}
                      onRequestInfo={handleRequestInfo}
                    />
                  ))}
